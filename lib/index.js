...
var Beandocs = function()
{
	...
	{     
		...
		var absPath = fs.realpathSync(ops.input);           

		var finder = findit.find(absPath),
		...
		{                                       
			if((file.split('.').pop() != 'js') || file.substr(file.indexOf(absPath) == 0 ? absPath.length : 0).indexOf('node_modules') > -1) return;
